...
<style>
  ...
  .collapsible-card .card-header-left {
    display: flex;
    align-items: center;
    min-width: 0;
  }
  ...
  .queue-refresh-indicator.updating {
    color: #1976ff;
    font-weight: 600;
  }
...
<div class="dashboard-watermark">
  <div class="container py-4">
    ...
    <div class="row g-4">
      {{-- LEFT COLUMN --}}
      <div class="col-lg-6">
        <div class="dashboard-card collapsible-card" data-collapsible>
          <div class="card-header-toggle" role="button" tabindex="0" aria-expanded="true">
          ...
          <div class="collapsible-body mt-3">
            @if($upcoming->isEmpty())
        ...
        @if(!Auth::user()->is_admin && !Auth::user()->is_secretary)
          ...
          @if($activeQueues->count() > 0)
            <div class="dashboard-card collapsible-card" id="queue-card" data-collapsible data-autorefresh-card>
              <div class="card-header-toggle" role="button" tabindex="0" aria-expanded="true">
                <div class="card-header-left">
                  <h5 class="mb-0">
                ...
                <div class="d-flex align-items-center gap-2 flex-wrap justify-content-end">
                  <span class="queue-refresh-indicator" id="queue-refresh-indicator">
                    <i class="bi bi-arrow-clockwise me-1" id="queue-refresh-icon"></i>
                    Auto-refresh on
                  </span>
                  <span class="collapse-icon">
                    <i class="bi bi-chevron-down"></i>
              ...
              <div class="collapsible-body mt-3" id="queue-card-content">
                <div class="list-group list-group-flush">
                    ...
                    <div class="list-group-item border-0 px-0 py-2">
                      <div class="d-flex align-items-center justify-content-between">
                        ...
                        <div class="text-end">
                          ...
                          <div class="mt-1">
                            <a href="{{ route('queue.status.entry', $queueEntry) }}" class="btn btn-sm btn-outline-primary">
          ...
          @else
            <div class="dashboard-card collapsible-card" data-collapsible>
              <div class="card-header-toggle" role="button" tabindex="0" aria-expanded="true">
                <div class="card-header-left">
                  <h5 class="mb-0">
                    <i class="bi bi-info-circle medical-icon me-2"></i>Queue Information
                  </h5>
                </div>
                <span class="collapse-icon">
                  <i class="bi bi-chevron-down"></i>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  collapsibleCards.forEach((card, index) => {
    ...
    if (!card.id) {
      card.id = 'collapsible-card-' + index;
    }

    const storageKey = 'dashboard_' + card.id + '_collapsed';
  ...
  const queueContent = document.getElementById('queue-card-content');
  const refreshIndicator = document.getElementById('queue-refresh-indicator');
  const refreshIcon = document.getElementById('queue-refresh-icon');

  if (queueCard && queueContent) {
    let isRefreshing = false;

    const refreshQueueCard = async function () {
      if (document.hidden || isRefreshing) return;
      if (queueCard.classList.contains('collapsed')) return;

      isRefreshing = true;

      if (refreshIndicator) refreshIndicator.classList.add('updating');
      if (refreshIcon) refreshIcon.classList.add('rotate-refresh');
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error('Failed to refresh queue card.');
        }

        const html = await response.text();
        const parser = new DOMParser();
        const doc = parser.parseFromString(html, 'text/html');
        const newQueueContent = doc.getElementById('queue-card-content');

        if (newQueueContent) {
          queueContent.innerHTML = newQueueContent.innerHTML;
        }
      } catch (error) {
        console.error('Queue auto-refresh error:', error);
      } finally {
        isRefreshing = false;
        if (refreshIndicator) refreshIndicator.classList.remove('updating');
        if (refreshIcon) refreshIcon.classList.remove('rotate-refresh');
      }
    };

    setInterval(refreshQueueCard, 15000);
  }
});
</script>
@else
