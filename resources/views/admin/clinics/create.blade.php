          ...
          <form method="POST" action="{{ route('admin.clinics.store') }}" enctype="multipart/form-data">
            ...
            <div class="mb-3 location-search-wrapper">
              ...
              <small class="text-muted">
                <i class="bi bi-info-circle me-1"></i>
                Search for the clinic location to pin it on the map. The coordinates will be saved automatically.
              </small>
            ...
            <div class="mb-3">
              ...
              <div id="mapPicker" class="border rounded"></div>
            </div>
            ...
            <input type="hidden"
                   id="gps_latitude"
                   name="gps_latitude"
                   class="@error('gps_latitude') is-invalid @enderror"
                   value="{{ old('gps_latitude', old('latitude')) }}">
            @error('gps_latitude') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
            @error('latitude') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror

            <input type="hidden"
                   id="gps_longitude"
                   name="gps_longitude"
                   class="@error('gps_longitude') is-invalid @enderror"
                   value="{{ old('gps_longitude', old('longitude')) }}">
            @error('gps_longitude') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
            ...
            <div class="mb-3">
              ...
              <input type="file" name="logo" id="logoInput" class="form-control @error('logo') is-invalid @enderror" accept="image/jpeg,image/png,image/jpg,image/gif,image/webp">
...
function initializeMap() {
  ...
  const oldLat = parseFloat(latInput?.value || '{{ old('gps_latitude', old('latitude')) }}');
  const oldLng = parseFloat(lngInput?.value || '{{ old('gps_longitude', old('longitude')) }}');
