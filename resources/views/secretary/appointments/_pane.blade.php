...
<div class="medical-card p-4 mb-4">
  ...
  <form class="row g-3" method="GET" action="{{ route('secretary.dashboard') }}">
    <div class="col-lg-3">
    ...
    <div class="table-responsive">
      <table class="table table-hover align-middle mb-0">
        <thead class="bg-light">
          <tr>
            <th class="border-0 px-4 py-3"><i class="bi bi-person me-1"></i>Patient</th>
            <th class="border-0 px-4 py-3"><i class="bi bi-gear me-1"></i>Service</th>
            <th class="border-0 px-4 py-3"><i class="bi bi-person-badge me-1"></i>Doctor</th>
            <th class="border-0 px-4 py-3"><i class="bi bi-calendar me-1"></i>Date & Time</th>
            <th class="border-0 px-4 py-3"><i class="bi bi-info-circle me-1"></i>Status</th>
            <th class="border-0 px-4 py-3"><i class="bi bi-file-earmark-medical me-1"></i>Document</th>
        ...
        <tbody>
          @forelse($appointments as $appointment)
            <tr>
              <td class="px-4 py-3">
                <div class="d-flex align-items-center">
                  ...
                  <div>
                    ...
                    <small class="text-muted"><i class="bi bi-envelope me-1"></i>{{ $appointment->user->email }}</small>
                    @if($appointment->user->phone)
                      <br><small class="text-muted"><i class="bi bi-telephone me-1"></i>{{ $appointment->user->phone }}</small>
                    @endif
                  </div>
                </div>
              </td>
              ...
              <td class="px-4 py-3">
                <div class="fw-semibold"><i class="bi bi-calendar-date me-1"></i>{{ \Carbon\Carbon::parse($appointment->appointment_date)->format('M d, Y') }}</div>
              ...
              <td class="px-4 py-3">
                @if($appointment->medical_document)
                  <a href="{{ asset('storage/' . $appointment->medical_document) }}" target="_blank" class="btn btn-sm btn-outline-secondary">
      ...
      @forelse($appointments as $appointment)
        <div class="col">
          <div class="clinic-card h-100 p-4">
            ...
            <div class="mb-3">
              <div class="d-flex align-items-center mb-2"><i class="bi bi-building text-primary me-2"></i><strong>{{ $appointment->clinic->name }}</strong></div>
              <div class="d-flex align-items-center mb-2"><i class="bi bi-gear text-info me-2"></i><span>{{ $appointment->service->name }}</span></div>
              @if($appointment->doctor)
                <div class="d-flex align-items-center mb-2"><i class="bi bi-person-badge text-success me-2"></i><span>Dr. {{ $appointment->doctor->name }}</span></div>
              @endif
              <div class="d-flex align-items-center mb-2"><i class="bi bi-calendar-event text-warning me-2"></i><span>{{ \Carbon\Carbon::parse($appointment->appointment_date)->format('M d, Y') }}</span></div>
              <div class="d-flex align-items-center"><i class="bi bi-clock text-info me-2"></i><span>{{ \Carbon\Carbon::parse($appointment->appointment_time)->format('h:i A') }}</span></div>
              @if($appointment->medical_document)
                <div class="d-flex align-items-center mt-2"><i class="bi bi-file-earmark-medical text-secondary me-2"></i>
                  <a href="{{ asset('storage/' . $appointment->medical_document) }}" target="_blank">View Document</a>
                </div>
              @endif
            </div>
      ...
      @empty
        <div class="col-12">
          <div class="text-center py-5">
            ...
            <a href="{{ route('secretary.appointments.create') }}" class="btn btn-primary"><i class="bi bi-plus-circle me-2"></i>Create First Appointment</a>
          </div>
        </div>
      @endforelse
    </div>
