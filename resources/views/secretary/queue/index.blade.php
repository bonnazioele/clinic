...
<div class="container py-4">
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
      <div>
        <h2 class="fw-bold text-primary mb-1">
          <i class="bi bi-clock-history me-2"></i>Queue Management
        </h2>
        <p class="text-muted mb-0">{{ $clinic->name }} — {{ $clinic->address }}</p>
      </div>

      <form method="GET" action="{{ route('secretary.queue.index', $clinic) }}" class="d-flex gap-2 align-items-end">
        <div>
  ...
  <div class="medical-card p-4 mb-4">
    ...
    @if($waiting->isEmpty())
      <div class="text-center py-4">
        ...
        <h6 class="mt-3 mb-0">No active queue for {{ \Carbon\Carbon::parse($selectedDate)->format('M j, Y') }}</h6>
      </div>
    @else
              ...
              <tr>
                ...
                <td>
                  @if($queueEntry->appointment)
                    {{ \Carbon\Carbon::parse($queueEntry->appointment->appointment_date)->format('M j, Y') }}
  ...
  <div class="medical-card p-4">
    <div class="d-flex justify-content-between mb-3">
      <h5><i class="bi bi-check2-square me-2"></i>Completed / Finished Queue</h5>
      <span class="badge bg-success">{{ $completed->count() }} finished</span>
    </div>

    @if($completed->isEmpty())
      <p class="text-muted mb-0">No completed or finished queue entries for this date.</p>
    @else
      <div class="table-responsive">
        <table class="table table-hover align-middle">
          <thead class="bg-light">
          ...
          <tbody>
            @foreach($completed as $queueEntry)
              <tr>
                <td>#{{ $queueEntry->queue_number }}</td>
                <td>{{ $queueEntry->display_name }}</td>
                <td>
                  @if($queueEntry->appointment)
                    {{ \Carbon\Carbon::parse($queueEntry->appointment->appointment_date)->format('M j, Y') }}
                    at {{ $queueEntry->appointment->appointment_time }}
                ...
                <td>{{ $queueEntry->updated_at?->format('M j, Y g:i A') }}</td>
              </tr>
            @endforeach
          </tbody>
        </table>
  ...
  if (reschedModalEl) {
    ...
    document.querySelectorAll('[data-bs-target="#reschedModal"][data-action-url]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (url) {
          try { localStorage.setItem('lastReschedActionUrl', url); } catch(e) {}
        }
      });
    });
    ...
    if (hasErrors) {
      const stored = (() => {
        try { return localStorage.getItem('lastReschedActionUrl'); } catch(e) { return null; }
      })();
      const form = reschedModalEl.querySelector('form');
